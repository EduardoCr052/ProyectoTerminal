...
\section{Marco Teórico}
    ...
    \subsection{La corriente alterna}
        La corriente alterna (AC) es el tipo de corriente eléctrica predominante en las redes de distribución residencial a nivel mundial. Se dice que a  diferencia de la corriente continua (DC), donde el flujo de electrones es unidireccional, en la corriente alterna la magnitud y dirección varían periódicamente siguiendo una forma de onda sinusoidal \cite{sadiku2013}. Esta forma de onda se justifica por la eficiencia en la transmisión de energía y la naturaleza de los generadores rotativos.

        En el contexto nacional, la Norma Oficial Mexicana NOM-001-SEDE-2012 establece que la red eléctrica residencial suministra una tensión nominal de 127V a 60Hz \cite{nom2012}. Esta frecuencia implica que la señal completa se repite 60 veces por segundo, resultando en un periodo de $T \approx 16.67$ milisegundos por ciclo.

        La naturaleza oscilatoria de la corriente alterna introduce complejidades en la medición, ya que voltaje y corriente son funciones dependientes del tiempo $v(t)$ e $i(t)$, requiriendo técnicas de muestreo digital para su análisis \cite{boylestad2011}.

        \begin{figure}[H]
            \centering
            \includegraphics[width=0.7\textwidth]{img/AC.png}
            \caption[Representación de una señal de corriente alterna]{Representación de una señal de corriente alterna.}
            \label{fig:AC}
        \end{figure}

    \subsection{Fundamentos de Medición Eléctrica}

        Para la gestión eficiente de energía, es imperativo comprender la dinámica de las variables físicas. La amplitud define los valores máximos y mínimos de la señal, mientras que la frecuencia determina la velocidad de oscilación \cite{boylestad2011}.

        \begin{figure}[H]
            \centering
            \includegraphics[width=0.7\textwidth]{img/AC_amplitud.png}
            \caption[Amplitud de una señal de corriente alterna]{Amplitud de una señal de corriente alterna.}
            \label{fig:AC_amplitud}
        \end{figure}

    \subsubsection{Potencia Instantánea}
        La potencia instantánea se considera como el producto de la tensión y la corriente en un instante dado. Lo cual resulta muy util durante las oscilaciones y nos proporciona medios para calcular la potencia promedio. En un circuito de AC, la potencia no es estática \cite{hayt2012}, y se define como el producto del voltaje y la corriente en un instante específico:

        \begin{equation}
            p(t) = v(t) \cdot i(t)
        \end{equation}

        Dado que el sistema propuesto utiliza procesamiento digital en un microcontrolador ESP32, la señal continua se discretiza. Según la teoría de monitoreo digital documentada por OpenEnergyMonitor \cite{openenergymonitor}, la potencia instantánea para la muestra $n$-ésima se calcula como:

        \begin{equation}
            p[n] = v[n] \cdot i[n]
        \end{equation}

        \noindent \textit{Nota: En implementaciones donde no se cuenta con sensor de voltaje, $v[n]$ puede aproximarse como una constante $V_{rms}$, aunque la definición teórica rigurosa requiere el muestreo de ambas señales.}
    ...
    \subsubsection{Potencia Promedio (Potencia Activa)}
        ...
        Según explican Hayt, Kemmerly y Durbin \cite{hayt2012}, la potencia promedio se define matemáticamente como la integral de la potencia instantánea $p(t)$ sobre un periodo completo $T$, dividida por la duración de dicho periodo:
        ...
        \paragraph{Cálculo Discreto en el Microcontrolador} \hfill \break
        A diferencia de la Potencia Aparente (que se calcula multiplicando $V_{RMS} \cdot I_{RMS}$), la Potencia Activa real debe calcularse integrando muestra por muestra antes de promediar, para capturar correctamente el desfase entre voltaje y corriente.

        En el firmware del sistema propuesto, se aplica la sumatoria discreta documentada por las notas de aplicación de monitoreo energético \cite{programarfacil}:

        \begin{equation}
            P = \frac{1}{N} \sum_{n=0}^{N-1} (v[n] \cdot i[n])
        \end{equation}

        Donde $v[n]$ e $i[n]$ son las muestras instantáneas emparejadas. Este método es crucial porque permite medir el consumo real ($Watts$) independientemente del factor de potencia de la carga.
    ...
    \subsubsection{Valor Eficaz (RMS)}
        ...
        De acuerdo con Hayt, Kemmerly y Durbin \cite{hayt2012}, el valor RMS se define físicamente como el valor equivalente de una corriente directa (DC) que produciría la misma disipación de potencia promedio (calor) al circular por una resistencia óhmica idéntica. Matemáticamente, para una función continua en el tiempo $i(t)$, este valor se obtiene mediante la raíz cuadrada del promedio del cuadrado de la señal a lo largo de un periodo $T$:

        \begin{equation}
            I_{RMS} = \sqrt{\frac{1}{T} \int_{0}^{T} i^2(t) \, dt}
        \end{equation}

        Esta definición es relevante, ya que, vincula una señal oscilatoria con un valor escalar de potencia real, permitiendo comparar sistemas de AC y DC bajo un mismo estándar energético.

        Para la implementación del sistema de monitoreo, no es viable aplicar un promedio aritmético simple a la señal capturada. Como se detalla en la documentación técnica \cite{programarfacil}, la naturaleza sinusoidal de la corriente alterna presenta simetría entre sus semiciclos positivos y negativos. En consecuencia, la suma directa de todos los valores instantáneos durante un ciclo completo resultaría matemáticamente en cero, lo cual no refleja el flujo real de energía.
        ...
        Por lo tanto, la ecuación que rige el funcionamiento del algoritmo en el microcontrolador, sustituyendo la integral continua por una sumatoria de $N$ muestras, es:
    ...
    \subsubsection{Potencia Aparente ($S$)}
        ...
        Según definen Hayt, Kemmerly y Durbin \cite{hayt2012}, la Potencia Aparente ($S$) es el producto de los valores eficaces (RMS) de voltaje y corriente. El término "aparente" deriva del hecho de que, si se miden el voltaje y la corriente por separado con un multímetro convencional, el producto de estas lecturas parece ser la potencia total del sistema.
        ...
        \paragraph{Relación Vectorial y Relevancia en el Monitoreo} \hfill \break
        Desde la perspectiva de la teoría de circuitos, la potencia aparente corresponde a la magnitud vectorial de la potencia compleja, relacionando la potencia activa ($P$) y la potencia reactiva ($Q$) mediante el teorema de Pitágoras, conformando lo que se conoce como el triángulo de potencias:

        \begin{equation}
            S = \sqrt{P^2 + Q^2}
        \end{equation}

        \textbf{Implementación en el Sistema:}
    ...
    \subsubsection{Factor de potencia}
        El Factor de Potencia ($FP$) es el indicador de calidad y eficiencia más relevante en un sistema de corriente alterna. Se define como la relación adimensional entre la Potencia Activa ($P$), que realiza trabajo útil, y la Potencia Aparente ($S$), que es la energía total suministrada por la red.

        Según Alexander y Sadiku \cite{sadiku2013}, este parámetro representa qué porcentaje de la energía aparente se está transformando realmente en trabajo. Matemáticamente se expresa como:

        \begin{equation}
            FP = \frac{P}{S}
        \end{equation}

        En sistemas con cargas lineales y ondas sinusoidales puras, el factor de potencia equivale al coseno del ángulo de desfase ($\theta$) entre la onda de voltaje y la de corriente:

        \begin{equation}
            FP = \cos(\theta)
        \end{equation}

        \paragraph{Interpretación Física} \hfill \break
        El valor del $FP$ oscila entre 0 y 1.
        \begin{itemize}
            \item \textbf{$FP = 1$ (Unitario):} Ideal. Toda la energía suministrada se convierte en trabajo (cargas puramente resistivas).
            \item \textbf{$FP < 1$:} Indica ineficiencia debida a la presencia de Potencia Reactiva ($Q$). Esto ocurre comúnmente en cargas inductivas (motores, refrigeradores) que retrasan la corriente respecto al voltaje.
        \end{itemize}

        \paragraph{Cálculo Computacional} \hfill \break
        Para la implementación digital en el sistema de monitoreo, no se mide directamente el desfase temporal entre las ondas debido a la complejidad de detectar cruces por cero precisos en presencia de ruido eléctrico. En su lugar, tal como documenta la metodología de OpenEnergyMonitor \cite{openenergy}, el factor de potencia se deriva aritméticamente de los valores totales acumulados:

        \begin{equation}
            FP = \frac{\frac{1}{N}\sum (v[n] \cdot i[n])}{V_{RMS} \cdot I_{RMS}}
        \end{equation}

        Esta aproximación es computacionalmente eficiente y permite al sistema identificar cargas que degradan la calidad de la energía en la instalación residencial.
    \newpage
