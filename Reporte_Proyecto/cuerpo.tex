...
\section{Marco Teórico}
    ...
    \subsection{La corriente alterna}
        La corriente alterna (AC) es el tipo de corriente eléctrica predominante en las redes de distribución residencial a nivel mundial. Según Alexander y Sadiku \cite{sadiku2013}, a diferencia de la corriente continua (DC), donde el flujo de electrones es unidireccional, en la corriente alterna la magnitud y dirección varían periódicamente siguiendo una forma de onda sinusoidal. Esta forma de onda se justifica por la eficiencia en la transmisión de energía y la naturaleza de los generadores rotativos.
    ...
    \subsubsection{Potencia Instantánea}
        En un circuito de AC, la potencia no es estática. De acuerdo con Hayt et al. \cite{hayt2012}, la potencia instantánea $p(t)$ se define como el producto del voltaje y la corriente en un instante específico:

        \begin{equation}
            p(t) = v(t) \cdot i(t)
        \end{equation}

        Para una carga lineal con un desfase $\phi$, esta ecuación se expande mostrando un componente constante (potencia activa) y uno oscilatorio de frecuencia $2\omega$:
    ...
    \subsubsection{Valor Eficaz (RMS)}
        El valor eficaz o RMS (*Root Mean Square*) es el estándar para cuantificar la magnitud de una señal AC. Físicamente, representa el valor de corriente directa que produciría la misma disipación de calor por efecto Joule en una resistencia equivalente \cite{halliday2014}.

        A diferencia del promedio simple (que sería cero en una onda senoidal pura), el RMS cuantifica la capacidad energética real. Para la implementación digital, la integral continua se reemplaza por una suma de Riemann. Según las notas de aplicación para medidores de energía de estado sólido \cite{atmel2005}, la fórmula implementada en el firmware para $N$ muestras es:

        \begin{equation}
            I_{RMS} = \sqrt{\frac{1}{N} \sum_{n=0}^{N-1} i[n]^2}
        \end{equation}

        Donde $N$ es el número de muestras tomadas durante al menos un ciclo completo de red. Este método, conocido como "True RMS", permite medir con precisión incluso ante la presencia de ruido o distorsión armónica.

        \subsubsection{Potencia Aparente ($S$)}
        La Potencia Aparente ($S$) representa la capacidad total de suministro requerida por la red. Se define como el producto de los valores eficaces de tensión y corriente, sin considerar el ángulo de fase \cite{chapman2012}:

        \begin{equation}
            S = V_{RMS} \cdot I_{RMS}
        \end{equation}

        Su unidad es el Volt-Ampere (VA). En el contexto del monitoreo residencial, $S$ es una métrica crítica para dimensionar la carga total del sistema y detectar consumos fantasmas en los electrodomésticos.
